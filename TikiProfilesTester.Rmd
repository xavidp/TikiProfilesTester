...
purl("TikiProfilesTester.Rmd", documentation = 0)
```
...
if( any(!pCRAN %in% rownames(installed.packages())) ){
  install.packages(pCRAN[!pCRAN %in% rownames(installed.packages())], repos = 'http://cran.rstudio.com')
}
...
root@penguinbookpro:/var/www/tikiprofiletests/trunk# php console.php d:i --force;php console.php cache:clear --all;php console.php profile:apply Voting_System
...
root@penguinbookpro:/var/www/tikiprofiletests/trunk# 
...
for (vv in 1:length(Versions)) {
  ...
  cat(paste0("\n* Version: ", Versions[vv], " -------------------------------------- \n"))
  Revision <- system(paste0("cd ", basepath, Versions[vv],";svn info | grep Revision"), intern=TRUE)
...
#    for (ii in 1:2) {
  ...
  cat(paste0("** Profile (", ii, "/", dim(profilesList)[1],"): ", profilesList[ii,paste0("ProfileName_", Versions[vv])], "\n"))
    ...
    outputFilename[ii] <- paste0(Versions[vv], "_", format(Sys.Date(), "%y%m%d"), "_p", sprintf("%02d", ii), "_output_", profilesList[ii,paste0("ProfileName_", Versions[vv])], ".txt")
      command <- paste0("cd ", basepath, Versions[vv], "/;php console.php d:i --force > \"", file.path(getwd(), "log", format(Sys.Date(), "%y%m%d"), outputFilename[ii]), "\" 2>&1 ;php console.php cache:clear --all >> \"", file.path(getwd(), "log", format(Sys.Date(), "%y%m%d"), outputFilename[ii]), "\" 2>&1;php console.php profile:apply ", profilesList[ii,paste0("ProfileName_", Versions[vv])], " >> \"", file.path(getwd(), "log", format(Sys.Date(), "%y%m%d"), outputFilename[ii]), "\" 2>&1")
      system(command)  
